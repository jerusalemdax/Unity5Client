...
public sealed class BuglyAgent
{
    ...
    {
		UnityEngine.Debug.Log (string.Format("Bugly Crash Reporter: {0}, {1}, {2}", _crashReporterPackage, _crashReporterType, _crashReproterCustomizedLogLevel));
    }
    ...
    private static void ConfigCrashReporterType(){
        if (!_crashReporterTypeConfiged) {
            try {
                _BuglyConfigCrashReporterType(_crashReporterType); // MSDK
            } catch {
    ...
    private static int _crashReporterType = 0;  // Default=0,MSDK=2
